...
export const compressGlb = command('compress')
	...
	.action(async ({ args, flags, ctx, out }) => {
		...
		async function compressOne(inputPath: string): Promise<string | undefined> {
			...
			// A .gltf references its buffers and textures by relative uri, so they
			// resolve against the input file's own directory.
			const resourceResolver = /\.gltf$/i.test(inputPath)
				? async (uri: string): Promise<Uint8Array | undefined> => {
					...
					if (!(await pathExists(resourcePath))) return undefined;
