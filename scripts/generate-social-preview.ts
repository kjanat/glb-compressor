...
const REPO = 'github.com/kjanat/glb-compressor';
const [WIDTH, HEIGHT] = [1280, 640];
...
const svg = `<svg width="${WIDTH}" height="${HEIGHT}" xmlns="http://www.w3.org/2000/svg">
  ...
  <text x="${WIDTH - 40}" y="40" font-family="'SF Mono','Consolas',monospace" font-size="13" fill="#484f58" text-anchor="end">v${pkg.version}</text>
</svg>`;
...
export const link = (text: string, url: string): string => {
	const openLink: string = ((sequence: string) => {
		if (
	...
	const closeLink: string = ((sequence: string) => {
		if (
			process.env.TERM?.startsWith('screen') ||
			process.env.TERM?.startsWith('tmux') ||
			process.env.TMUX !== undefined
...
console.log(
	`Generated social-preview.png (${WIDTH}x${HEIGHT}, ${(stats.size / 1024).toFixed(1)} KB)`,
);
console.info(
	`Upload the image here: ${link(`${REPO}/settings/#repo-image-file-input`, `https://${REPO}/settings/#:~:text=release%20is%20published.-,Social%20preview,-Upload%20an%20image`)}`,
);
